...
class NFTdisplay extends Component {  
    ...
    const res = await nft.methods.tokenURI(this.props.id).call()
    
console.log("IPFS -------"+res);
let url="";
if(res.substring(0,4)=="ipfs"){url="https://gateway.ipfs.io"+res.substring(4);}else{url=res;}
    fetch(url)
      .then(response => response.json())
      .then(json => {   
                        this.setState({ res: JSON.stringify(json)});
                        this.props.updtLandscape(json["image"])
    ...
    render(){
       ...
       let type= (this.state.res)?JSON.parse(this.state.res)["type"]:null
       let color="red";
       if(type)color=panelColor[type]
       console.log("NFTD "+src)
       return(     
        <Minipanel id={this.props.id} name={name} contract={this.props.contract} src={src} color={color} width="100" height="100"/>
        ); 
    }   
}

 
  //getNFTURI(this.props)
export function getNFTURI(prop){
    return <p>hallo</p>
...
//<Minipanel id={id} name={name} url={ima} color={col} width={wid} height={heig}/> 
