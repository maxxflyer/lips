...
class Board extends Component {
    constructor(props) {
        super(props)
        this.state = { ipfs: '' , account: '' , thisM :[], count : 1}
        this.x=0
      }

      componentDidMount() {
        this.loadBlockchainData(0,[])
    }
    shouldComponentUpdate(){ //decidere flusso
        if(this.x++>5){return(false)}else{return(true)};  
    }

      async loadBlockchainData(id,arr) {

        let arra = arr;
        cons("BOARD LOAD:"+id,this.props)
        const c_addr=this.props.data[2][0];
        
        const nft = new web3.eth.Contract(ABI, c_addr)
        let res;

        try{res = await nft.methods.tokenOfOwnerByIndex(this.props.data[1][0],id).call();}catch(e){console.log("ERROR *** BOARD LOAD: VERY LIKELY THE CONTRACT IS MISSING THE _tokenOfOwnerByIndex_ FUNCTION")}

        if(res!=undefined)cons("RES: ",res.length)
        if((res!=0)&&(res.length<50)) //fix
        {            
            arra.push(res);
         
            this.loadBlockchainData(id+1,arra) 
        }else{
          cons.toString("STATE SET")
            this.setState({ thisM: arra , count : arra.length})//decidere flusso
        }       
    ...
    render(){ 
        ...
        cons("BOARD RENDER -----",this.state.thisM)
        ...
        return (<div id="board" style={{textAlign:"center", marginTop: "100px"}}><div id="address_line">{this.props.data[1]}</div><br/><br/><div id="empty">EMPTY</div></div>);
            if(this.state.count>0)
        return (<div id="board" style={{textAlign:"center", marginTop: "100px"}}>LOADING...{this.x}</div>);
        }else{
        return (
             <Display updtLandscape={this.props.updtLandscape} numbers={this.state.thisM} prp={this.props}/>            
        ); }
    }
...
function Display (props)  {
    ...
    const listItems = ids.map((number) =>
      <div key={number}><NFTdisplay updtLandscape={props.updtLandscape} contract={props.prp.data[2]} id={number}/></div>
    );
    return (
